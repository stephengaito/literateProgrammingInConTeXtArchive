...
assert.isEqual(macroBody[14],
  "    if thirddata.literateProgs.addTest ~= nil then")
assert.isEqual(macroBody[15], "      thirddata.literateProgs.addTest(")
assert.isEqual(macroBody[16], "        '_typing_'")
assert.isEqual(macroBody[17], "      )")
assert.isEqual(macroBody[18], "    else")
assert.isEqual(macroBody[19], "      thirddata.literateProgs.addCode(")
assert.isEqual(macroBody[20], "        'Test',")
assert.isEqual(macroBody[21], "        '_typing_'")
assert.isEqual(macroBody[22], "      )")
assert.isEqual(macroBody[23], "    end")
assert.isEqual(macroBody[24], "  }")
assert.isEqual(macroBody[25], "}")
assert.isEqual(macroBody[26], "\\unexpanded\\def\\createTestFile#1#2#3{%")
assert.isEqual(macroBody[27], "  \\directlua{")
assert.isEqual(macroBody[28], "    thirddata.literateProgs.createCodeFile(")
assert.isEqual(macroBody[29], "      'Test',")
assert.isEqual(macroBody[30], "      '#1',")
assert.isEqual(macroBody[31], "      '#2',")
assert.isEqual(macroBody[32], "      '#3'")
assert.isEqual(macroBody[33], "    )")
assert.isEqual(macroBody[34], "  }")
assert.isEqual(macroBody[35], "}")
assert.isEqual(macroBody[36], "\\unexpanded\\def\\setTestStream#1{%")
assert.isEqual(macroBody[37], "  \\directlua{")
assert.isEqual(macroBody[38], "    thirddata.literateProgs.setCodeStream(")
assert.isEqual(macroBody[39], "      'Test',")
assert.isEqual(macroBody[40], "      '#1'")
assert.isEqual(macroBody[41], "    )")
assert.isEqual(macroBody[42], "  }")
assert.isEqual(macroBody[43], "}")
assert.isEqual(macroBody[44], "\\unexpanded\\def\\prependTest#1{%")
assert.isEqual(macroBody[45], "  \\directlua{")
assert.isEqual(macroBody[46], "    thirddata.literateProgs.setPrepend(")
assert.isEqual(macroBody[47], "      'Test',")
assert.isEqual(macroBody[48], "      '#1'")
assert.isEqual(macroBody[49], "    )")
assert.isEqual(macroBody[50], "  }")
assert.isEqual(macroBody[51], "}")
\stopLuaTest
\stopTestCase
...
\begingroup
  \defineLitProgs[TestCode]
  \directlua{
    local function addTestCode(aBufferName)
      thirddata.literateProgs.addCode('SpecificTestCode', aBufferName)
    end
    thirddata.literateProgs.addTestCode = addTestCode
  }
  \traceContextMacro{doStartTestCode}{1}
  \startTestCode
    this is some test "code"
